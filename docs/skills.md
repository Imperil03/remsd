...
2. `AGENTS.md`.
3. `.agent/SEO_STRUCTURE.md`, существующий контент сайта и явно указанные источники фактов.
...
- `global-design-skill` - внешний MIT design operating system от `github.com/staurus86/global-design-skill`; использовать как расширенный справочник по UI/UX, дизайн-токенам, accessibility, responsive, SEO-дизайну и handoff.
...
- `global-design-skill` - полностью из `github.com/staurus86/global-design-skill`, MIT.
...
2. Сверить факты с существующим контентом сайта, документами в проекте и явно указанными пользователем источниками.
3. Для SEO/GEO-текстов открыть `.agent/skills/infostyle/references/semantic-seo-authorship.md`.
4. Применить `infostyle`.
5. Применить `russian-text-humanization`.
6. Для публичных SEO/коммерческих текстов подключить reviewer subagent: он проверяет естественность языка, читабельность, полезность, SEO-структуру и фактологическую безопасность.
...
- `.agent/agents/remsd-copywriter.md` — пишет текст по `SEO_STRUCTURE -> существующие факты и источники -> semantic-seo-authorship -> infostyle -> humanization`.
...
- `global-design-skill` не перебивает B2B-стиль РемСД, подтвержденные факты и SEO-структуру; использовать его как источник правил и чеклистов, а не как разрешение на неподтвержденные claims.
