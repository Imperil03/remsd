...
Репозиторий: [Imperil03/remsd](https://github.com/Imperil03/remsd), рабочая ветка `main`. Для перехода с другого компьютера см. [инструкцию запуска и итог последнего выпуска](docs/project-handoff.md). Все необходимые решения и источники находятся в Git; прежняя переписка и локальные worktree не нужны.
...
Если меняются hero, показатели компании или шрифты, обновить critical CSS:
...
Push в `main` запускает сборку, полный браузерный прогон и Lighthouse перед выкладкой. Проверять нужно успешный deployment конкретного коммита и сам публичный URL. Последний проверенный выпуск и примечания к проверкам записаны в `docs/project-handoff.md`.
...
Согласованный итог от 25.09.2026: показатель `1 000 м² / площадь ремонтной базы`; список возможностей под адресом базы; компактные «Условия ремонта» с двумя пунктами без нумерации. Значение площади не связано с количеством страниц марок.
