...
- **Composition:** dark split hero with copy left and the real engine-work photo right; flat `10+ / 500+ / 23 / 7` facts; three undated history stages; a pale workshop section with three photos and four capabilities; a real team photo beside four specialty rows; dark approach copy beside four principles; three official brands above three document groups; dark contacts beside the Yandex map. Captions stay outside photos.
- **Responsive layout:** team, approach and contacts stack at `≤1020 px`; hero, history and workshop photos stack at `≤720 px`. Facts become `2 × 2` at `≤720 px` without section padding. Document groups use two columns of horizontal cards above `720 px` and one at `≤720 px`; the conformity document always occupies one wide card. Official brands form a single list of horizontal rows at `≤720 px`.
- **Documents and viewing:** previews use contained images without stretching or cropping. Document titles use `18 px`, then `17 px` at `≤720 px` and `16 px` at `≤360 px`. The conformity certificate and five appendices remain one six-image group. The dark native dialog provides a caption, counter, direct file link, finite previous/next controls, arrow keys, Escape, contained focus and return focus. Single-image groups hide the step controls; load errors retain the file link. The mobile callbar hides while the viewer is open.
...
**The Document Catalog Rule.** Home and About share one catalog of unchanged scans, with purpose and neutral date information visible before opening a document. About separates service powers, specialist training and service conformity while preserving the official-brand strip. Dates retain the source's expiry, issue date or stated duration; they receive no automatic extension or validity badge. Stable document IDs connect previews, anchors and viewer groups.

- **Full catalog:** MAZ and Advers form the service-powers group; URAL and Weichai form specialist training; conformity has one certificate with five appendices. Horizontal cards pair a contained scan with its title, purpose, date and viewing action. The preview column is `128 px` above `1120 px`, `100 px` at `721–1120 px`, `88 px` at `361–720 px` and `68 px` at `≤360 px`.
- **Home and navigation:** two horizontal previews show MAZ and conformity in two columns above `720 px`, one below. Their preview columns are `96 px`, `80 px` at `≤720 px` and `64 px` at `≤360 px`. Each card links to `/o-kompanii/#document-{id}`. The general link and footer's “Документы и сертификаты” link open `/o-kompanii/#documents`; the top-menu “Сертификаты” item also opens that block; no separate certificate page is created. Header breakpoints remain unchanged.

`src/data/documents.json` owns groups, purpose, dates and pages; About selects `documentIds`, and the existing viewer uses `documents-{id}` groups. See `docs/documents.md`.
