...
На wide desktop типы техники показываются сеткой `3 × 2`: фотография `16:9` сверху, название `17 px` и компактное пояснение `14 px` снизу. На `≤1020 px` сетка становится двухколоночной, а на `≤520 px` — горизонтальными строками без фиксированной высоты. Подписи не накладываются на фотографии.

### Service Cards and Brand Strip

Шесть направлений ремонта образуют сетку `3 × 2`: горизонтальная карточка использует предметную line-иконку `56 px`, заголовок `17 px` и компактное описание. При сужении сетка переходит в две, затем в одну колонку. Логотипы не получают описания или ссылки до публикации брендовых страниц.
