...
**The Air Between Systems Rule.** Разные смысловые системы разделяются поверхностью и секционным отступом. Связанные сетки услуг, техники и марок собираются на общей белой поверхности и разделяются общим `64 px`-ритмом и реальными ссылками-переходами.
