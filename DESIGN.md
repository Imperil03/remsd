...
- **Headline:** секционные заголовки внутреннего hub используют `36–40 px` на wide desktop, `32–36 px` на tablet и `30–32 px` на mobile.
- **Title:** названия карточек и этапов используют `18–20 px` и вес 700–850.
- **Body:** основной B2B-текст и описания карточек не мельче `16 px`, с длиной строки до 70 знаков там, где это возможно.
...
Основной контейнер ограничен шириной 1180 px, а фото-hero использует более широкий shell до 1360 px. `section-default` разделяет крупные главы, а `section-compact` используется внутри связок «услуги → техника → марки», для этапов и индекса связанных разделов. На mobile все внутренние секции переходят на компактный ритм.

На wide desktop (`>1120 px`) шесть услуг, шесть типов техники и девять марок образуют по одному цельному ряду, как в утверждённом блочном референсе. При `1021–1120 px` услуги и техника переходят в три колонки, марки — в центрированную стену `5 + 4`; при `721–1020 px` остаются две колонки карточек и три колонки марок, а этапы становятся вертикальным маршрутом. На `≤520 px` услуги и техника становятся одноколоночными горизонтальными карточками без фиксированной высоты, марки — двухколоночными. Навигация переходит в burger при 1120 px. Горизонтальный scroll недопустим начиная с 320 px.
