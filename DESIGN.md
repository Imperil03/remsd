...
Wide desktop использует три колонки для услуг и техники, пять для этапов и максимум пять для марок. При ширине `721–1020 px` карточки услуг и техники переходят в две колонки, марки — в три, этапы — в вертикальный маршрут. На `≤520 px` услуги и техника становятся одноколоночными горизонтальными карточками без фиксированной высоты, марки — двухколоночными. Навигация переходит в burger при 1120 px. Горизонтальный scroll недопустим начиная с 320 px.
...
Типы техники показываются равноправной сеткой `3 × 2`: фотография `16:9` сверху, название `20 px` и пояснение `16 px` снизу, высота карточки от `300 px`. На tablet сетка становится двухколоночной, на `≤520 px` — компактными горизонтальными строками без фиксированной высоты. Подписи не накладываются на фотографии.

### Service Cards and Brand Strip

Шесть направлений ремонта образуют desktop-сетку `3 × 2`: карточка имеет высоту от `210 px`, предметную line-иконку `56 px`, заголовок `19 px` и описание `16 px`. Марки образуют центрированную сетку `5 + 4` шириной до `900 px`; ячейка имеет высоту от `104 px`, оптическое поле логотипа — до `148 × 64 px`. Логотипы не получают описания или ссылки до публикации брендовых страниц.

### Symptoms and Work Stages

Признаки неисправности собираются в рамочную сетку `3 × 2` рядом с первой тёмной телефонной CTA-панелью шириной `340 px`; карточки имеют высоту от `96 px`, иконки `40 px`, текст `16 px`. Пять этапов ремонта показываются как связанный маршрут `01–05` с икон-плитками `64 px`, заголовками `18 px` и amber-соединителем; на tablet/mobile маршрут становится вертикальным.
