...
try {
  require("./lib/check-certificates-page")({ root, catalog: internalCatalog });
} catch (error) {
  fail(`certificates: ${error.message}`);
}
...
if (!fs.existsSync(distDir)) {
  ...
  for (const page of internalCatalog.pages) {
    ...
    if (!["contact", "documents"].includes(page.family) && !/<section\b[^>]*class=["'][^"']*internal-close\b/i.test(html)) fail(`${relative}: нет заключительного CTA`);
