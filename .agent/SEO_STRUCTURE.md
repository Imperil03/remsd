...
В шапке также допустимы адрес и email как NAP-сигналы:
- `Сургут, Домостроителей, 13`;
- `info@remsd.ru`.

Эти данные не являются отдельным навигационным слоем. Если шапка визуально перегружена, их можно убрать из hero-шапки, но оставить в контактах, футере и структурированных данных.
...
Визуальное правило для этой группы: не дублировать отдельную строку `Официальный сервис` под заголовком `Марки грузовиков`. Официальный статус КАМАЗ, МАЗ и УРАЛ передавать порядком, компактным маркером и `aria-label`, чтобы меню не выглядело перегруженным.
...
- в меню использовать обычный текст, а в каталоге — статическую плитку без hover, focus и роли ссылки;
- будущий адрес можно хранить только в контентной модели, но не выводить в HTML до публикации страницы.
...
Будущие направления можно показывать как неинтерактивные элементы:
- `/arenda/avtokranov/`;
